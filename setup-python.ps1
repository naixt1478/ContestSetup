...
function Install-PythonDirect {
    ...
    Write-Progress -Activity $PA -Status "Checking existing installation..." -PercentComplete 10
    ...
    if (-not (Test-Path $PythonExe)) {
        if (-not (Test-Path $PythonInstaller)) {
            Write-Progress -Activity $PA -Status "Downloading Python..." -PercentComplete 30
        ...
        Write-Progress -Activity $PA -Status "Installing Python..." -PercentComplete 60
