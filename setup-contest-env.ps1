...
function Invoke-VSCodeUninstallerIfPresent {
    $InstallFolders = @(
    ...
    foreach ($Folder in $InstallFolders) {
        ...
        $Uninstaller = Get-ChildItem -Path $Folder -Filter "unins*.exe" -ErrorAction SilentlyContinue |
            Select-Object -First 1

        if ($Uninstaller) {
            Write-Host "Running VS Code uninstaller: $($Uninstaller.FullName)"
            ...
            Write-Host "VS Code uninstaller exit code: $($Process.ExitCode)"
        }
    }
}
...
function Reset-VSCodeCompletely {
    ...
    Invoke-VSCodeUninstallerIfPresent
...
function Install-VSCodeDirect {
    ...
    $Process = Start-Process `
        ...
        -ArgumentList @(
            "/VERYSILENT",
            "/NORESTART",
            "/MERGETASKS=addcontextmenufiles,addcontextmenufolders,addtopath"
        ) `
        -Wait `
        -PassThru
...
# ============================================================
# 1. Create folders and start logging
# ============================================================
...
Write-Section "2. Environment diagnostics"
...
if (Test-CommandExists "winget") {
    Write-Host "winget found. Version:"
    winget --version
    Write-Host "winget is not used as the primary installer in this script." -ForegroundColor Yellow
}
else {
    Write-Host "winget not found. Direct installers will be used." -ForegroundColor Yellow
}
